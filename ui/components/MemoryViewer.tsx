...
function ConfidenceBar({ value }: { value: number }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-[10.5px] tabular-nums text-faint">{pct}%</span>
    </div>
  );
}
...
export default function MemoryViewer({ workspaceId }: { workspaceId: string }) {
  ...
  return (
    <Card className="p-4">
      ...
      {adding && (
        <div className="mb-3 space-y-2 rounded-xl border border-line bg-surface-2/60 p-3">
          ...
          <textarea
            ...
            placeholder="value"
            rows={2}
            className="focus-ring w-full rounded-lg border border-line bg-surface-2 px-2.5 py-1.5 text-[12px] text-fg placeholder:text-faint"
          />
      ...
      {loading ? (
        <p className="py-6 text-center text-[12px] text-faint">Loading memory…</p>
      ) : items.length === 0 ? (
          ...
          {items.map((m) => (
            <li key={m.id} className="rounded-xl border border-line bg-surface p-3">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="truncate font-mono text-[12px] font-medium text-fg">{m.key}</span>
                  </div>
                  <p className="mt-1 text-[12.5px] leading-relaxed text-body">{m.value}</p>
                </div>
                <button
                  onClick={() => onForget(m.id)}
                  title="Forget this"
                  className="shrink-0 rounded-md p-1 text-faint transition hover:bg-rose-50 hover:text-rose-500"
                >
              ...
              <div className="mt-2 flex items-center justify-between">
                ...
                <span className="text-[10.5px] text-faint">used {m.last_used?.slice(0, 10)}</span>
              </div>
            </li>
          ))}
        </ul>
